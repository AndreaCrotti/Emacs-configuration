  ...
  (mapc
   (lambda (entry)
     (puthash
      ...
      cache))
   (dictree--complete-ranked-cache dict-latex-docclass))
  (setf (dictree--complete-ranked-cache dict-latex-docclass)
        cache))
(let ((cache (make-hash-table :test 'equal))
      (trie (dictree--trie dict-latex-docclass)))
  (mapc
   (lambda (entry)
     (puthash
      ...
      (dictree--cache-create
       ...
       (dictree--cache-maxnum (cdr entry)))
      cache))
   (dictree--regexp-ranked-cache dict-latex-docclass))
  (setf (dictree--regexp-ranked-cache dict-latex-docclass)
        cache))
(unless (memq dict-latex-docclass dictree-loaded-list)
  (push dict-latex-docclass dictree-loaded-list))
