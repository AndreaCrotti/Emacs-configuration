  ...
  (mapc
   (lambda (entry)
     (puthash
      ...
      cache))
   (dictree--complete-ranked-cache dict-latex-math-bbm))
  (setf (dictree--complete-ranked-cache dict-latex-math-bbm)
        cache))
(let ((cache (make-hash-table :test 'equal))
      (trie (dictree--trie dict-latex-math-bbm)))
  (mapc
   (lambda (entry)
     (puthash
      ...
      (dictree--cache-create
       ...
       (dictree--cache-maxnum (cdr entry)))
      cache))
   (dictree--regexp-ranked-cache dict-latex-math-bbm))
  (setf (dictree--regexp-ranked-cache dict-latex-math-bbm)
        cache))
