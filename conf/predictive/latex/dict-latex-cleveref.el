  ...
  (mapc
   (lambda (entry)
     (puthash
      ...
      cache))
   (dictree--complete-ranked-cache dict-latex-cleveref))
  (setf (dictree--complete-ranked-cache dict-latex-cleveref)
        cache))
(let ((cache (make-hash-table :test 'equal))
      (trie (dictree--trie dict-latex-cleveref)))
  (mapc
   (lambda (entry)
     (puthash
      ...
      (dictree--cache-create
       ...
       (dictree--cache-maxnum (cdr entry)))
      cache))
   (dictree--regexp-ranked-cache dict-latex-cleveref))
  (setf (dictree--regexp-ranked-cache dict-latex-cleveref)
        cache))
(unless (memq dict-latex-cleveref dictree-loaded-list)
  (push dict-latex-cleveref dictree-loaded-list))
