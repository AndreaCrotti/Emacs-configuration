  ...
  (mapc
   (lambda (entry)
     (puthash
      ...
      cache))
   (dictree--complete-ranked-cache dict-html-map))
  (setf (dictree--complete-ranked-cache dict-html-map)
        cache))
(let ((cache (make-hash-table :test 'equal))
      (trie (dictree--trie dict-html-map)))
  (mapc
   (lambda (entry)
     (puthash
      ...
      (dictree--cache-create
       ...
       (dictree--cache-maxnum (cdr entry)))
      cache))
   (dictree--regexp-ranked-cache dict-html-map))
  (setf (dictree--regexp-ranked-cache dict-html-map)
        cache))
