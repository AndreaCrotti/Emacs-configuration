  ...
  (mapc
   (lambda (entry)
     (puthash
      ...
      cache))
   (dictree--complete-ranked-cache dict-html-form))
  (setf (dictree--complete-ranked-cache dict-html-form)
        cache))
(let ((cache (make-hash-table :test 'equal))
      (trie (dictree--trie dict-html-form)))
  (mapc
   (lambda (entry)
     (puthash
      ...
      (dictree--cache-create
       ...
       (dictree--cache-maxnum (cdr entry)))
      cache))
   (dictree--regexp-ranked-cache dict-html-form))
  (setf (dictree--regexp-ranked-cache dict-html-form)
        cache))
(unless (memq dict-html-form dictree-loaded-list)
  (push dict-html-form dictree-loaded-list))
