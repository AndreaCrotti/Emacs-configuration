  ...
  (mapc
   (lambda (entry)
     (puthash
      ...
      cache))
   (dictree--complete-ranked-cache dict-html-optgroup))
  (setf (dictree--complete-ranked-cache dict-html-optgroup)
        cache))
(let ((cache (make-hash-table :test 'equal))
      (trie (dictree--trie dict-html-optgroup)))
  (mapc
   (lambda (entry)
     (puthash
      ...
      (dictree--cache-create
       ...
       (dictree--cache-maxnum (cdr entry)))
      cache))
   (dictree--regexp-ranked-cache dict-html-optgroup))
  (setf (dictree--regexp-ranked-cache dict-html-optgroup)
        cache))
(unless (memq dict-html-optgroup dictree-loaded-list)
  (push dict-html-optgroup dictree-loaded-list))
