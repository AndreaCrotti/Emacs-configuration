  ...
  (mapc
   (lambda (entry)
     (puthash
      ...
      cache))
   (dictree--complete-ranked-cache dict-html-col))
  (setf (dictree--complete-ranked-cache dict-html-col)
        cache))
(let ((cache (make-hash-table :test 'equal))
      (trie (dictree--trie dict-html-col)))
  (mapc
   (lambda (entry)
     (puthash
      ...
      (dictree--cache-create
       ...
       (dictree--cache-maxnum (cdr entry)))
      cache))
   (dictree--regexp-ranked-cache dict-html-col))
  (setf (dictree--regexp-ranked-cache dict-html-col)
        cache))
(unless (memq dict-html-col dictree-loaded-list)
  (push dict-html-col dictree-loaded-list))
