  ...
  (mapc
   (lambda (entry)
     (puthash
      ...
      cache))
   (dictree--complete-ranked-cache dict-html-international))
  (setf (dictree--complete-ranked-cache dict-html-international)
        cache))
(let ((cache (make-hash-table :test 'equal))
      (trie (dictree--trie dict-html-international)))
  (mapc
   (lambda (entry)
     (puthash
      ...
      (dictree--cache-create
       ...
       (dictree--cache-maxnum (cdr entry)))
      cache))
   (dictree--regexp-ranked-cache dict-html-international))
  (setf (dictree--regexp-ranked-cache dict-html-international)
        cache))
(unless (memq dict-html-international dictree-loaded-list)
  (push dict-html-international dictree-loaded-list))
