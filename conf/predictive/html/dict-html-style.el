  ...
  (mapc
   (lambda (entry)
     (puthash
      ...
      cache))
   (dictree--complete-ranked-cache dict-html-style))
  (setf (dictree--complete-ranked-cache dict-html-style)
        cache))
(let ((cache (make-hash-table :test 'equal))
      (trie (dictree--trie dict-html-style)))
  (mapc
   (lambda (entry)
     (puthash
      ...
      (dictree--cache-create
       ...
       (dictree--cache-maxnum (cdr entry)))
      cache))
   (dictree--regexp-ranked-cache dict-html-style))
  (setf (dictree--regexp-ranked-cache dict-html-style)
        cache))
