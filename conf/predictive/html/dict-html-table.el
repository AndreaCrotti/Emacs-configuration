  ...
  (mapc
   (lambda (entry)
     (puthash
      ...
      cache))
   (dictree--complete-ranked-cache dict-html-table))
  (setf (dictree--complete-ranked-cache dict-html-table)
        cache))
(let ((cache (make-hash-table :test 'equal))
      (trie (dictree--trie dict-html-table)))
  (mapc
   (lambda (entry)
     (puthash
      ...
      (dictree--cache-create
       ...
       (dictree--cache-maxnum (cdr entry)))
      cache))
   (dictree--regexp-ranked-cache dict-html-table))
  (setf (dictree--regexp-ranked-cache dict-html-table)
        cache))
(unless (memq dict-html-table dictree-loaded-list)
  (push dict-html-table dictree-loaded-list))
