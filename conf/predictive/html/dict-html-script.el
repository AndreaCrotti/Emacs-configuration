  ...
  (mapc
   (lambda (entry)
     (puthash
      ...
      cache))
   (dictree--complete-ranked-cache dict-html-script))
  (setf (dictree--complete-ranked-cache dict-html-script)
        cache))
(let ((cache (make-hash-table :test 'equal))
      (trie (dictree--trie dict-html-script)))
  (mapc
   (lambda (entry)
     (puthash
      ...
      (dictree--cache-create
       ...
       (dictree--cache-maxnum (cdr entry)))
      cache))
   (dictree--regexp-ranked-cache dict-html-script))
  (setf (dictree--regexp-ranked-cache dict-html-script)
        cache))
(unless (memq dict-html-script dictree-loaded-list)
  (push dict-html-script dictree-loaded-list))
