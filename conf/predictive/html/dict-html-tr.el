...
(let ((cache (make-hash-table :test 'equal))
      (trie (dictree--trie dict-html-tr)))
  (mapc
   (lambda (entry)
     (puthash
      (car entry)
      ...
      cache))
   (dictree--complete-ranked-cache dict-html-tr))
  (setf (dictree--complete-ranked-cache dict-html-tr)
        cache))
(let ((cache (make-hash-table :test 'equal))
      (trie (dictree--trie dict-html-tr)))
  (mapc
   (lambda (entry)
     (puthash
      ...
      (dictree--cache-create
       ...
       (dictree--cache-maxnum (cdr entry)))
      cache))
   (dictree--regexp-ranked-cache dict-html-tr))
  (setf (dictree--regexp-ranked-cache dict-html-tr)
        cache))
