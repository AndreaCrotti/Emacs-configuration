  ...
  (mapc
   (lambda (entry)
     (puthash
      ...
      cache))
   (dictree--complete-ranked-cache dict-html-del))
  (setf (dictree--complete-ranked-cache dict-html-del)
        cache))
(let ((cache (make-hash-table :test 'equal))
      (trie (dictree--trie dict-html-del)))
  (mapc
   (lambda (entry)
     (puthash
      ...
      (dictree--cache-create
       ...
       (dictree--cache-maxnum (cdr entry)))
      cache))
   (dictree--regexp-ranked-cache dict-html-del))
  (setf (dictree--regexp-ranked-cache dict-html-del)
        cache))
(unless (memq dict-html-del dictree-loaded-list)
  (push dict-html-del dictree-loaded-list))
