  ...
  (mapc
   (lambda (entry)
     (puthash
      ...
      cache))
   (dictree--complete-ranked-cache dict-html-option))
  (setf (dictree--complete-ranked-cache dict-html-option)
        cache))
(let ((cache (make-hash-table :test 'equal))
      (trie (dictree--trie dict-html-option)))
  (mapc
   (lambda (entry)
     (puthash
      ...
      (dictree--cache-create
       ...
       (dictree--cache-maxnum (cdr entry)))
      cache))
   (dictree--regexp-ranked-cache dict-html-option))
  (setf (dictree--regexp-ranked-cache dict-html-option)
        cache))
(unless (memq dict-html-option dictree-loaded-list)
  (push dict-html-option dictree-loaded-list))
