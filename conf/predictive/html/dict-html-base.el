  ...
  (mapc
   (lambda (entry)
     (puthash
      ...
      cache))
   (dictree--complete-ranked-cache dict-html-base))
  (setf (dictree--complete-ranked-cache dict-html-base)
        cache))
(let ((cache (make-hash-table :test 'equal))
      (trie (dictree--trie dict-html-base)))
  (mapc
   (lambda (entry)
     (puthash
      ...
      (dictree--cache-create
       ...
       (dictree--cache-maxnum (cdr entry)))
      cache))
   (dictree--regexp-ranked-cache dict-html-base))
  (setf (dictree--regexp-ranked-cache dict-html-base)
        cache))
(unless (memq dict-html-base dictree-loaded-list)
  (push dict-html-base dictree-loaded-list))
