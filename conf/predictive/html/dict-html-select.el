  ...
  (mapc
   (lambda (entry)
     (puthash
      ...
      cache))
   (dictree--complete-ranked-cache dict-html-select))
  (setf (dictree--complete-ranked-cache dict-html-select)
        cache))
(let ((cache (make-hash-table :test 'equal))
      (trie (dictree--trie dict-html-select)))
  (mapc
   (lambda (entry)
     (puthash
      ...
      (dictree--cache-create
       ...
       (dictree--cache-maxnum (cdr entry)))
      cache))
   (dictree--regexp-ranked-cache dict-html-select))
  (setf (dictree--regexp-ranked-cache dict-html-select)
        cache))
(unless (memq dict-html-select dictree-loaded-list)
  (push dict-html-select dictree-loaded-list))
