  ...
  (mapc
   (lambda (entry)
     (puthash
      ...
      cache))
   (dictree--complete-ranked-cache dict-html-quote))
  (setf (dictree--complete-ranked-cache dict-html-quote)
        cache))
(let ((cache (make-hash-table :test 'equal))
      (trie (dictree--trie dict-html-quote)))
  (mapc
   (lambda (entry)
     (puthash
      ...
      (dictree--cache-create
       ...
       (dictree--cache-maxnum (cdr entry)))
      cache))
   (dictree--regexp-ranked-cache dict-html-quote))
  (setf (dictree--regexp-ranked-cache dict-html-quote)
        cache))
(unless (memq dict-html-quote dictree-loaded-list)
  (push dict-html-quote dictree-loaded-list))
