...
#+begin_src emacs-lisp :tangle yes
  ...
  ;TODO: make this thing more general if possible
  (add-hook 'conf-unix-mode-hook
            '(lambda ()
               (local-set-key (kbd "M-n") 'ca-next-conf-section)
               (local-set-key (kbd "M-p") 'ca-prev-conf-section)))
  
  ;TODO: this should be done as soon as possible to make it easy to override
  ;; whenever we are in /etc try to set the conf mode
