...
#+begin_src emacs-lisp :tangle yes
  ...
  (defun dired-git-current ()
    (dired-git-files :directory desktop-dirname))
  ;; first we have to pass to the right dir
  (add-hook 'desktop-after-read-hook 'dired-git-gcurrent)
  ...
  (setq desktop-clear-preserve-buffers
        (append '("miniconf.org" "eternal" "\\*shell\\*" "\\*group\\*" "\\*ielm\\*") desktop-clear-preserve-buffers))
#+end_src
...
*** Filtering variables to avoid always adding
#+begin_src emacs-lisp :tangle yes
  ...
  (defun is-filtered-org ()
    (-org-agenda-is-filtered org-agenda-filter-out))
  
  (defun -org-agenda-is-filtered (filter)
    (if
        (string-match (car filter) org-agenda-filter-out)
        t)
    (org-agenda-filter-out (cdr filter)))
#+end_src
...
#+begin_src emacs-lisp :tangle yes
  ...
  (global-set-key [f7] 'dired-git-files)
